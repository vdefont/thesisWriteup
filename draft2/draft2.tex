...
We then compare the three flows for each weighting method, where the flows are represented as their fraction of the whole matrix Y, using the Frobenius norm with respect to the $C^1$ inner product $<A,B> = \sum_{i,j}A_{ij}B_{ij}W_{ij}$.
